...
def clean_and_validate(df: pd.DataFrame, model, drop_duplicates_by=None, not_null_fields=None):
    ...
    for _, row in df.iterrows():
        try:
            ...
            for key, value in row_data.items():
                if value is None and hasattr(model.__annotations__.get(key), '__origin__'):
                    if model.__annotations__[key].__origin__ in (int, float):
                        row_data[key] = 0
    ...
    # удаляем NULL в обязательных полях
    if not_null_fields:
...
def consume_and_save(batch_size: int = 100, timeout: int = 30):
    ...
    insert_events = """
        INSERT INTO raw.events (user_id, session_id, event_type, product_id, order_id, quantity,
                                city, country, lat, lon, cost_usd, cost_eur, cost_rub,
                                weather_temp, weather_desc, timestamp, product_category)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """

    insert_profiles = "INSERT INTO raw.user_profiles (user_id, name, email, birth_date, gender) VALUES (%s,%s,%s,%s,%s)"
    insert_devices = "INSERT INTO raw.user_devices (user_id, device_type, os, browser) VALUES (%s,%s,%s,%s)"
    insert_locations = "INSERT INTO raw.user_locations (user_id, city, country, lat, lon) VALUES (%s,%s,%s,%s,%s)"
    insert_orders = "INSERT INTO raw.orders (order_id, user_id, order_ts, total_usd) VALUES (%s,%s,%s,%s)"
    insert_order_items = "INSERT INTO raw.order_items (order_id, product_id, quantity, price_usd) VALUES (%s,%s,%s,%s)"
    insert_campaigns = "INSERT INTO raw.campaign_events (user_id, campaign_id, campaign_name, channel, action, generated_at) VALUES (%s,%s,%s,%s,%s,%s)"
    insert_order_status = "INSERT INTO raw.order_status_events (order_id, status, generated_at) VALUES (%s, %s, %s)"
...
def _flush_buffers(cur, buffers, insert_events, insert_profiles,
    ...
    if buffers["campaigns"]:  # ← ДОБАВЛЕНО
